...
double RbtRealGrid::GetValue(const RbtCoord &c) const {
  return isValid(c) ? m_grid(GetIX(c), GetIY(c), GetIZ(c)) : 0.0;
}

double RbtRealGrid::GetValue(unsigned int iX, unsigned int iY,
...
void RbtRealGrid::SetValue(const RbtCoord &c, double val) {
  if (isValid(c))
    m_grid(GetIX(c), GetIY(c), GetIZ(c)) = val;
}
void RbtRealGrid::SetValue(unsigned int iX, unsigned int iY, unsigned int iZ,
                           double val) {
  if (isValid(iX, iY, iZ))
    m_grid(iX, iY, iZ) = val;
}
