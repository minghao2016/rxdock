...
TEST_F(RbtChromTest, PopulationRWFitness) {
  ...
  ASSERT_LT(fabs(lastValue - 1.0), TINY);
}
...
TEST_F(RbtChromTest, RandomiseOccupancy) {
  ...
  meanDiff *= stepSize;
  maxDiff *= stepSize;
  ASSERT_LT(fabs(minDiff), 0.01);
  ASSERT_LT(fabs(meanDiff - 0.5), 0.01);
  ASSERT_LT(fabs(maxDiff - 1.0), 0.01);
}
...
TEST_F(RbtChromTest, OccupancyThreshold) {
  ...
  ASSERT_LT(fabs(enabledProb - occupancyProb), 0.01);
}
